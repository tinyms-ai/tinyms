...
def load_pretrained_model(path='torch2ms/ms_weight', model=None):
    print(f"Loading model from {path}")

    param_not_load = []

    unet_weight = ms.load_checkpoint(os.path.join(path, 'unet.ckpt'))
    param_not_load.extend(ms.load_param_into_net(model.model, unet_weight))

    vae_weight = ms.load_checkpoint(os.path.join(path, 'vae.ckpt'))
    param_not_load.extend(ms.load_param_into_net(model.first_stage_model, vae_weight))
...
def main(opts):
    ...
    optimizer = build_optimizer(CLDMWithLoss, opts, lr)
    ...
    if opts.use_parallel:
        net_with_grads = ParallelTrainOneStepWithLossScaleCell(CLDMWithLoss, optimizer=optimizer,          
                                                               scale_sense=update_cell, parallel_config=ParallelConfig)
    else:
        net_with_grads = TrainOneStepWithLossScaleCell(CLDMWithLoss, optimizer=optimizer,
