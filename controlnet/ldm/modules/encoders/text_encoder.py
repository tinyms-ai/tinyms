...
class MultiheadSelfAttention(nn.Cell):
    def __init__(self, d_model, n_head, dtype=ms.float32):
        ...
        self.k_proj = nn.Dense(self.embed_dim, self.embed_dim).to_float(dtype)
        self.v_proj = nn.Dense(self.embed_dim, self.embed_dim).to_float(dtype)
        self.q_proj = nn.Dense(self.embed_dim, self.embed_dim).to_float(dtype)
...
class TextEncoder(nn.Cell):
    def __init__(self,
        ...
        self.embedding_table = Parameter(initializer(TruncatedNormal(0.02), [vocab_size, width], dtype=ms.float32))
        self.gather = ops.Gather()
        self.reshape = ops.Reshape()
        self.cast = ops.Cast()

        self.positional_embedding = Parameter(initializer(TruncatedNormal(0.01), [context_length, width], dtype=ms.float32))
    ...
    def construct(self, text):
        ...
        # ok
        # print(f'input_embedding: {gather_result.sum()}, {gather_result.min()}, {gather_result.max()}')
        x = self.reshape(gather_result, (bsz, ctx_len, -1))
        # print(f'embeddings: {self.positional_embedding.sum()}, {self.positional_embedding.min()}, {self.positional_embedding.max()}')
