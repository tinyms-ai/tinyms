...
from densenet import densenetBC_100

# build the network
net = densenetBC_100(num_classes=10)
net.update_parameters_name(prefix='Ascend')
model = Model(net)
...
cifar10_ckpt_folder = 'densenet_BC_100'
cifar10_ckpt_path = 'densenet_BC_100/densenet.ckpt'

if not os.path.exists(cifar10_ckpt_folder):
    os.makedirs(cifar10_ckpt_folder)
else:
    print('densenet_BC_100 ckpt folder already exists')
...
ckpoint_cb = ModelCheckpoint(prefix="densenet_BC_100", config=CheckpointConfig(
