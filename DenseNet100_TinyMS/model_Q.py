...
import tinyms as ts
from tinyms import layers, Tensor, ms_function
...
class _Transition(layers.Layer):
    ...
    def __init__(self, num_input_features, num_output_features, avgpool=False):
        ...
        if avgpool:
            poollayer = layers.AvgPool2d(kernel_size=2, stride=2)
        else:
            poollayer = layers.MaxPool2d(kernel_size=2, stride=2)
    ...
    def __init__(self, growth_rate, block_config, num_init_features=None, bn_size=4, drop_rate=0):
        ...
        if num_init_features:
            self.features = layers.SequentialLayer([
                ...
                layers.ReLU(),
                layers.MaxPool2d(kernel_size=3, stride=2, pad_mode='same')
            ])    
...
def _densenet100(**kwargs):
    # add"num_init_features=24", due to input size = (224, 224). In fact, input size should be (32, 32)////////////
    return Densenet(growth_rate=12, block_config=(16, 16, 16), num_init_features=24, **kwargs)
